...
class QuestionnaireView(discord.ui.View):
    def __init__(self, client, mission_id, current_round=0, student_mission_info=None, timeout=None):
        ...
        self.click_options = set() or self.student_mission_info.get('clicked_options', set())
        ...
        for idx, option in enumerate(self.options):
            button = discord.ui.Button(
                ...
                style=discord.ButtonStyle.primary if option not in self.click_options else discord.ButtonStyle.secondary
            )
        ...
        self.submit_button = discord.ui.Button(
            ...
            disabled=True if len(self.click_options) < self.max_selections else False
        )
    ...
    def generate_summary_embed(self):
        description = ""
        for round_idx, record in enumerate(self.click_options):
            description += (
    ...
    def create_callback(self, idx):
        async def callback(interaction: discord.Interaction):
            ...
            try:
                ...
                if selected_option in self.click_options:
                    # remove selection if already selected
                    self.click_options.remove(selected_option)
                    self.children[idx].style = discord.ButtonStyle.primary
                    if len(self.click_options) < self.max_selections:
                        self.children[-1].disabled = True
                else:
                    if len(self.click_options) < self.max_selections:
                        self.click_options.add(selected_option)
                        self.children[idx].style = discord.ButtonStyle.secondary

                content = f"已選擇 {len(self.click_options)}/{self.max_selections} 個選項"
                if len(self.click_options) >= 3: # Must click 3 times to confirm
    ...
    async def submit_callback(self, interaction: discord.Interaction):
        ...
        try:
            save_questionnaire_record(str(interaction.user.id), str(self.message.id), self.mission_id, self.current_round, self.click_options)
